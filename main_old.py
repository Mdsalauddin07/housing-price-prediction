...
# 3.we will work on the copy of training data
...
# 5. list the  numerical and categorical columns 
num_attribs = housing.drop("ocean_proximity", axis= 1).columns.to_list()
cat_attribs = ["ocean_proximity"]

# 6. lets make the pipeline fro numerical columns 
num_pipeline = Pipeline([
    ("impute", SimpleImputer(strategy="median")),
    ("scaler", StandardScaler())
])

# 7. lets make the pipeline fro categorical columns
...
print(f"The root mean squared error for Design Tree is {dec_rmse}")
...
random_forest_rmse = root_mean_squared_error(housing_labels, dec_preds)
